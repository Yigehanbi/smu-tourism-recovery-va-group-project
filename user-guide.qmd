...
This guide explains how to use the final tourism analytics app as one connected workflow. The three analytical modules share the same country-arrivals backbone, so the intended reading order is simple: begin with market structure, continue into recovery-pattern comparison, and finish with forward projection.
...
Open the `Time Series Visual Analysis` tab when you want to establish market structure before moving into clustering or forecasting. The left control panel sets the ranking period, the map year inside that period, and the number of leading markets shown in the ranking view.

The `Market Map` page shows annual country totals on a choropleth map. Read the overview cards first, then use the selected-year insight panel to connect the map leader to the chosen ranking window. This page answers which markets dominate a given year and how concentrated the visible recovery landscape is.

The `Top Markets` page compares the pre-COVID and recovery leaders side by side. The ranking table and interpretation card explain how the leading source markets change between the two periods. This is the fastest way to see whether the same countries still lead in the recovery window and whether the market structure has shifted.
...
The `Model Studio` tab explains why one model wins. `Model leaderboard` ranks the models under `RMSE`, `MAE`, or `MAPE`. `Holdout residual comparison` shows where forecast errors remain on the holdout window. `Engine status` reports which engine was requested, which one actually executed, and which packages are missing when the strict `Require modeltime` path cannot run. `Model interpretation` turns the ranking result into a plain-language explanation.

### Diagnostics tab

The `Diagnostics` tab focuses on the time-series structure. `Raw time series` shows the full historical path. `Seasonal pattern` reveals month-level recurrence across years. `Decomposition` separates the series into trend, seasonal, and remainder components. `Split summary` confirms the training and testing window used in the latest run.
